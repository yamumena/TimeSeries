...
```{r}
ts_data_dif<- diff(ts_data, lag = 30)
...
* Se observa que el valor p es 0.02. El cual es menor a el nivel de significancia de 0.05. Esto sugiere que es improbable que la serie de tiempo tenga una raíz unitaria, lo que indica estacionariedad en la serie diferenciada.
...
1. Alpha = 0.42. Indica que se le da más peso a las observaciones recientes para la estimación del nivel.

2. Beta = 0.00. Indica que no se está considerando la componente de tendencia en el modelo Holt-Winters.

3. Gamma 0.73. Indica que se da un peso significativo a las observaciones recientes para la estimación de la estacionalidad.
...
Para realizar predicciones, utilizaremos tanto la función predict como forecast, y analizaremos las diferencias entre ellas.

Empleamos la función predict para generar predicciones para los próximos 360 pasos (días, apro un año) en el tiempo, e incluimos intervalos de predicción con un nivel de confianza del 95%.

```{r}

predicciones <- predict(hw_model, n.ahead = 360, prediction.interval = TRUE, level=0.95)
...
plot(ts_data_dif, main = "Predicciones Holt-Winters", xlab = "Tiempo", ylab = "Temperatura", xlim = c(2014, 2019))
lines(hw_model$fitted[,1], lty=2, col="blue")
lines(predicciones[,1], col="red")
lines(predicciones[,2], lty=2, col="orange")
lines(predicciones[,3], lty=2, col="orange")
```

* Como se indica, las líneas negras representan los datos de nuestra serie temporal. Las líneas azules representan el modelo ajustado. Las líneas rojas representan las predicciones para los próximos 30 días. Las líneas naranjas representan los intervalos de confianza asociados con estas predicciones. Como se puede observar, los intervalos se desbordan, lo que indica una alta incertidumbre en nuestras predicciones.

* A pesar de que a partir d elos intervalos se observa una alta incertidumbre, el año que fue pronosticado parece estar ajustado siguiendo la estacionalidad que hubo en los años conocidos.
...
* Como podemos observar, la predicción con la función forecast parece ser bastante similar al resultado anterior. En este caso, es más fácil notar cómo los intervalos de confianza se desbordan.
...
acf(hw_forecast$residuals, lag.max=30, na.action=na.pass)
